#!/usr/bin/env python3
"""Fix DEX genesis configuration with proper types and reward rate."""

import json
import sys

def fix_dex_genesis(genesis_path):
    """Fix the DEX module configuration in genesis with correct types and values."""
    
    with open(genesis_path, 'r') as f:
        genesis = json.load(f)
    
    # Complete DEX configuration with correct types and values
    genesis['app_state']['dex'] = {
        "params": {
            ...
            "base_reward_rate": "0.222",  # Correct rate for 7% annual
            "lc_denom": "ulc"
        },
    ...
    print("✓ DEX genesis configuration fixed with:")
    print("  - Correct types (numbers not strings)")
    print("  - Base reward rate: 0.222 (7% annual)")
    print("  - Trading pairs: MC/TUSD, MC/LC, USDC/TUSD")
    print("  - 8 liquidity tiers with proper windows")
